...
class Det3DLocalVisualizerCustom(DetLocalVisualizer):
    """MMDetection3D Local Visualizer Custom for Error seg.
    ...
    def draw_bev_bboxes(self,
        ...
        Args:
            ...
            line_styles (str or List[str]): The linestyle of lines.
                ...
                ?highlight=collection#matplotlib.collections
                .AsteriskPolygonCollection.set_linestyle
    ...
    def draw_seg_mask(self, seg_mask_colors: np.ndarray) -> None:
        ...
        if hasattr(self, 'pcd'):
            ...
            # offset = 102.4 * 1.2  # nus semkitti
            offset = 150.4 * 1.2  # waymo
    ...
    def show(self,
        ...
        if hasattr(self, 'o3d_vis'):
            # self.view_control.set_zoom(0.35)  # 0.3 kitti 0.35 other
            ...
            self.view_port = \
                self.view_control.convert_to_pinhole_camera_parameters()  #
            # noqa: E501
            if wait_time != -1:
    ...
    def add_datasample(self,
        ...
        if (vis_task == 'lidar_seg' and ignore_index is not None and
                'pts_semantic_mask' in data_sample.gt_pts_seg):  # noqa: E501
            keep_index = (data_sample.gt_pts_seg.pts_semantic_mask !=
                          ignore_index)  # noqa: E501
        else:
            keep_index = None
        ...
        if draw_gt and data_sample is not None:
            ...
            if 'gt_pts_seg' in data_sample and vis_task == 'lidar_seg':
                ...
                # code to only show raw point cloud
                # points = data_input['points']
                # check_type('points', points, (np.ndarray, Tensor))
                #
                # points = tensor2ndarray(points)
                ...
                # seg_color = points[:, :3]
                #
                # self.set_points(seg_color, pcd_mode=2, vis_mode='add',
                #                 mode='xyz')
        ...
        if draw_error and data_sample is not None:
            if ('pred_pts_seg' in data_sample and 'gt_pts_seg' in data_sample
                    and vis_task == 'lidar_seg'):
                assert classes is not None, 'class information is ' \
                                            'not provided when ' \
                                            'visualizing semantic ' \
                                            'segmentation results.'
                ...
                points = data_input['points']
                check_type('points', points, (np.ndarray, Tensor))
                # convert points and masks to array
                points = tensor2ndarray(points)
                gt_pts_sem_mask = tensor2ndarray(
                    data_sample.gt_pts_seg.pts_semantic_mask)
                pred_pts_sem_mask = tensor2ndarray(
                    data_sample.pred_pts_seg.pts_semantic_mask)

                if keep_index is not None:
                    keep_index = tensor2ndarray(keep_index)
                    points = points[keep_index]
                    gt_pts_sem_mask = gt_pts_sem_mask[keep_index]
                    pred_pts_sem_mask = pred_pts_sem_mask[keep_index]

                match_idx = pred_pts_sem_mask == gt_pts_sem_mask
                error_palette = np.array([[1.0, 0.0, 0.0], [0.5, 0.5, 0.5]])

                pts_color = error_palette[match_idx.astype(np.int64)]
                seg_color = np.concatenate([points[:, :3], pts_color], axis=1)

                if hasattr(self, 'pcd'):
                    offset = (np.array(self.pcd.points).max(0) -
                              np.array(self.pcd.points).min(0))[0] * 2.4
                    # offset = 102.4 * 2.4  # nus kitti
                    offset = 150.4 * 2.4  # waymo
                    mesh_frame = geometry.TriangleMesh.create_coordinate_frame(
                        size=1, origin=[offset, 0,
                                        0])  # create coordinate frame for seg
                    self.o3d_vis.add_geometry(mesh_frame)
                ...
                seg_points = copy.deepcopy(seg_color)
                seg_points[:, 0] += offset
                self.set_points(seg_points, pcd_mode=2, vis_mode='add',
                                mode='xyzrgb')
