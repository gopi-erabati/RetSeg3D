...
class Det3DDataPreprocessorCustom(DetDataPreprocessor):
    ...
    def __init__(self,
        ...
        super(Det3DDataPreprocessorCustom, self).__init__(
            mean=mean,
            std=std,
    ...
    def voxelize(self, points: List[Tensor],
        ...
        if self.voxel_type == 'hard':
            ...
            for i, res in enumerate(points):
                res_coors = self.voxel_layer(res)  # (N, 3)
                res_coors = F.pad(res_coors, (1, 0), mode='constant', value=i)
                coors.append(res_coors)
            voxels = torch.cat(points, dim=0)
            coors = torch.cat(coors, dim=0)
        elif self.voxel_type == 'dynamicretseg3d':
            coors = []
            # dynamic voxelization only provide a coors mapping
            for i, (res, data_sample) in enumerate(zip(points, data_samples)):
                res_coors = self.voxel_layer(res)  # (N, 3) but [z,y,x]
                # flip to produce [x, y, z]
                res_coors = torch.flip(res_coors, [1])  # (N, 3) [x,y,z]
                # get the GT voxel semantic seg mask
                self.get_voxel_seg(res_coors, data_sample)
                res_coors = F.pad(res_coors, (1, 0), mode='constant', value=i)
                # (N, 4) [b,x,y,z]
                coors.append(res_coors)
            voxels = torch.cat(points, dim=0)  # (BN, 4)
            coors = torch.cat(coors, dim=0)  # (BN, 4) [b,x,y,z]
            ...
            for i, (res, data_sample) in enumerate(zip(points, data_samples)):
                ...
                # get point and vox coords for unique voxel indices
                res_voxels, res_voxel_coors = res[inds], res_coors[inds]
                if hasattr(data_sample.gt_pts_seg, 'pts_semantic_mask'):
                    labels = data_sample.gt_pts_seg.pts_semantic_mask[inds]
                if self.training and self.max_voxels is not None:
                    if labels.shape[0] > self.max_voxels:
                        # if len(inds) > self.max_voxels:
                        #     inds = np.random.choice(
                        #         inds, self.max_voxels, replace=False)
                        init_idx = np.random.randint(labels.shape[0])
                        crop_idx = np.argsort(
                            np.sum(np.square(res_voxels.cpu().numpy() - res_voxels.cpu().numpy()[init_idx]),
                                   1))[
                                   :self.max_voxels]
                        res_voxels, res_voxel_coors, labels = (
                            res_voxels[crop_idx],
                            res_voxel_coors[crop_idx]
                            , labels[crop_idx])
                if hasattr(data_sample.gt_pts_seg, 'pts_semantic_mask'):
                    data_sample.gt_pts_seg.voxel_semantic_mask = labels
                if self.batch_first:
