...
class SemanticKittiDatasetCustom(Seg3DDataset):
    ...
    def __init__(self,
                 ...
                 **kwargs) -> None:
        metainfo['seg_label_mapping_inv'] = self.get_seg_label_mapping_inv(
            metainfo)
        super().__init__(
            data_root=data_root,
            ann_file=ann_file,
    ...
    def get_seg_label_mapping_inv(self, metainfo):
        seg_label_mapping_inv = np.zeros(len(metainfo[
                                                 'seg_label_mapping_inv']),
                                         dtype=np.uint32)
        for idx in metainfo['seg_label_mapping_inv']:
            seg_label_mapping_inv[idx] = metainfo['seg_label_mapping_inv'][idx]
        return seg_label_mapping_inv